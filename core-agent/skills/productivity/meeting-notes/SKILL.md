---
name: meeting-notes
description: Process meeting transcripts into structured action items, decisions, and summaries.
---

# meeting-notes

Process meeting transcripts into structured action items, decisions, and summaries.

## Overview
This skill provides automated workflows for meeting-notes.

## Instructions
1. Review the requirements and configuration options.
2. Provide necessary inputs and set parameters.
3. The skill will automatically execute the required steps.
4. Review the generated output and apply as needed.

## Configuration
Configure the necessary settings to run this skill effectively.
