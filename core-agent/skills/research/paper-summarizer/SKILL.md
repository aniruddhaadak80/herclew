---
name: paper-summarizer
description: Summarize academic papers into structured briefs with key findings, methodology, and implications.
---

# paper-summarizer

Summarize academic papers into structured briefs with key findings, methodology, and implications.

## Overview
This skill provides automated workflows for paper-summarizer.

## Instructions
1. Review the requirements and configuration options.
2. Provide necessary inputs and set parameters.
3. The skill will automatically execute the required steps.
4. Review the generated output and apply as needed.

## Configuration
Configure the necessary settings to run this skill effectively.
